...
class ERC20LoanUnlockCollateralModal extends Component {
    ...
    async componentDidMount() {
        ...
        const response = await FIL.decodeVoucher(loanDetails?.filCollateral?.unlockSignedVoucher)
    ...
    handleRedeemBtn = async (e) => {
        ...
        this.confirmOpInterval = setInterval(async () => {
            confirmRedeemUnlockCollateralVoucher({ CID: response?.payload?.CID['/'], network: shared?.filNetwork })
                .then((data) => data.json())
                .then((res) => {
                    console.log(res)
        ...
        return (
            ...
            >
                ...
                <div style={{ padding: '24px 48px', height: '100%', }}>
                    ...
                    {
                        modalState != 2 && modalState != 4 && modalState != 6 && modalState != 8 &&
                        <Stepper
                            activeStep={
                                modalState == 3 ? 1 :
...

                    {
                        ...
                        <Fragment>
                            <div className="mt-4" style={{ fontWeight: 500, fontSize: 16, textAlign: 'justify' }}>
                                You are unlocking your collateral ({loanDetails?.filCollateral?.amount} FIL).
                            </div>

                            <div className="mt-4" style={{ textAlign: 'justify' }}>
                                <div>• The Borrower has accepted your repayment and revealed secretB1 which will allow you to unlock your collateral from the FIL Payment Channel.</div>
                            ...
                            <div className="mt-4">
                                <div>Secret revealed by Borrower (secretB1)</div>
                                <div className="mt-2" style={{ fontWeight: 600, fontSize: 14, overflowWrap: 'break-word' }}>{web3.utils.toUtf8(loanDetails?.erc20Loan?.secretB1)}</div>
                            </div>
...

                    {
                        ...
                        <Fragment>
                            ...
                            <div style={{ padding: '0px 24px 36px 24px', display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column' }}>
                                ...
                                <div style={{ fontWeight: 500, fontSize: 16, marginTop: 5, textAlign: 'center' }}>Redeeming a voucher to unlock {loanDetails?.filCollateral?.amount} FIL from Payment Channel</div>
...

                    {
                        ...
                        <Fragment>
                            ...
                            <div style={{ padding: '0px 24px 24px 24px', display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column' }}>
                                ...
                                <div style={{ fontWeight: 400, fontSize: 16, marginTop: 25, marginBottom: 15, textAlign: 'center' }}>You have redeemed a voucher to unlock your collateral. To continue with the unlocking process, you'll need to `settle` the Payment Channel.</div>
...
export default connect(mapStateToProps)(ERC20LoanUnlockCollateralModal)