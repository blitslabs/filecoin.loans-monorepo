...
// API
import { confirmRedeemPaybackVoucher, confirmSettlePayback, confirmCollectPayback } from '../../../utils/api'

Modal.setAppElement('#root')
const web3 = new Web3()
BigNumber.set({ EXPONENTIAL_AT: 25 })

class FILLoanAcceptPaybackModal extends Component {
    ...
    async componentDidMount() {
        ...
        const response = await FIL.decodeVoucher(loanDetails?.filPayback?.signedVoucher)
        this.setState({
    ...
    handleRedeemBtn = async (e) => {
        e.preventDefault()
        const { shared, filecoin_wallet, protocolContracts, loanDetails, dispatch, loanId } = this.props
        const { password, secretB1 } = this.state
        const collateralLockContract = protocolContracts?.[shared?.networkId]?.ERC20CollateralLock?.address
        ...
        this.confirmOpInterval = setInterval(async () => {
            confirmRedeemPaybackVoucher({ CID: response?.payload?.CID['/'], network: shared?.filNetwork })
                .then((data) => data.json())
                .then((res) => {
                    console.log(res)
    ...
    handleSettleBtn = async (e) => {
        ...
        if (response?.status !== 'OK') {
            toast.success(response?.message, { position: "top-right", autoClose: 5000, hideProgressBar: false, closeOnClick: true, pauseOnHover: true, draggable: true, progress: undefined, });
        ...
        // Save Settle Tx
        this.confirmOpInterval = setInterval(async () => {
            confirmSettlePayback({ CID: response?.payload?.CID['/'], network: shared?.filNetwork })
                .then((data) => data.json())
                .then((res) => {
                    console.log(res)
    ...
    handleCollectBtn = async (e) => {
        ...
        // Save Settle Tx
        this.confirmOpInterval = setInterval(async () => {
            confirmCollectPayback({ CID: response?.payload?.CID['/'], network: shared?.filNetwork })
                .then((data) => data.json())
                .then((res) => {
                    console.log(res)
        ...
        return (
            ...
            >
                ...
                <div style={{ padding: '24px 48px', height: '100%', }}>
                    <div className="modal-title mt-2 text-center">ACCEPT PAYBACK</div>
...

                    {
                        ...
                        <Fragment>
                            <div className="mt-4" style={{ fontWeight: 500, fontSize: 16, textAlign: 'justify' }}>
                                You are accepting the borrowers's payback ({loanDetails?.filPayback?.amount} FIL).
                            </div>

                            <div className="mt-4" style={{ textAlign: 'justify' }}>
                                <div>• The Borrower has signed a voucher to allow you to withdraw the loan's payback from the FIL Payment Channel.</div>
                                <div className="mt-2 " style={{ fontWeight: 'bold' }}>• Make sure the voucher amount is correct before redeeming the voucher; otherwise, you'll reveal your preimage (secretB1) and withdraw an incorrect amount.</div>
                            ...
                            <div className="mt-4">
                                <div>Voucher's Secret Hash (secretHashB1)</div>
                                <div className="mt-2" style={{ fontWeight: 600, fontSize: 14, overflowWrap: 'break-word' }}>{`0x${secretHashB1}`}</div>
                            </div>
...

                    {
                        ...
                        <Fragment>
                            <div className="mt-4" style={{ fontWeight: 500, fontSize: 16, textAlign: 'justify' }}>
                                You are accepting the Borrower's payback ({loanDetails?.filPayback?.amount} FIL).
                            </div>

                            <div className="mt-4" style={{ textAlign: 'justify' }}>
                                <div>• The Borrower has signed a voucher to allow you to withdraw the payback amount from the FIL Payment Channel.</div>
                                <div className="mt-2 " style={{ fontWeight: 'bold' }}>• Make sure the voucher amount is correct before redeeming the voucher; otherwise, you'll reveal your preimage (secretB1) and withdraw an incorrect amount.</div>
...

                    {
                        ...
                        <Fragment>
                            <div className="mt-4" style={{ fontWeight: 500, fontSize: 16, textAlign: 'justify' }}>
                                You are settling the payment channel to withdraw the loan's payback ({loanDetails?.filPayback?.amount} FIL).
                            </div>
...
export default connect(mapStateToProps)(FILLoanAcceptPaybackModal)