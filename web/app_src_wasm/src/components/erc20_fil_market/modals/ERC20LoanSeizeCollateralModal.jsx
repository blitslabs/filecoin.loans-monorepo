...
// API
import { confirmRedeemSeizeCollateralVoucher, confirmSettleSeizeCollateral, confirmCollectSeizeCollateral } from '../../../utils/api'
...
class ERC20LoanSeizeCollateralModal extends Component {
    ...
    async componentDidMount() {
        ...
        const response = await FIL.decodeVoucher(loanDetails?.filCollateral?.signedVoucher)
    ...
    handleRedeemBtn = async (e) => {
        ...
        const response = await filecoin.redeemVoucher(
            ...
            Buffer.from(web3.utils.toUtf8(loanDetails?.erc20Loan?.secretA1)), // secretA1
        ...
        this.confirmOpInterval = setInterval(async () => {
            confirmRedeemSeizeCollateralVoucher({ CID: response?.payload?.CID['/'], network: shared?.filNetwork })
                .then((data) => data.json())
                .then((res) => {
                    console.log(res)
    ...
    handleSettleBtn = async (e) => {
        ...
        // Save Settle Tx
        this.confirmOpInterval = setInterval(async () => {
            confirmSettleSeizeCollateral({ CID: response?.payload?.CID['/'], network: shared?.filNetwork })
                .then((data) => data.json())
                .then((res) => {
                    console.log(res)
                    ...
                    if (res.status === 'OK') {
                        ...
                        dispatch(saveTx({
                            ...
                            txHash: response?.payload?.CID['/'],
                            from: filecoin_wallet?.public_key?.[shared?.filNetwork],
                            summary: `Settle Seize Payment Channel`
                        }))
    ...
    handleCollectBtn = async (e) => {
        ...
        // Save Settle Tx
        this.confirmOpInterval = setInterval(async () => {
            confirmCollectSeizeCollateral({ CID: response?.payload?.CID['/'], network: shared?.filNetwork })
                .then((data) => data.json())
                .then((res) => {
                    console.log(res)
        ...
        return (
            ...
            >
                ...
                <div style={{ padding: '24px 48px', height: '100%', }}>
                    ...
                    {
                        modalState != 1 && modalState != 2 && modalState != 4 && modalState != 8 &&
                        <Stepper
                            activeStep={
                                modalState == 3 ? 1 :
...

                    {
                        ...
                        <Fragment>
                            <div className="mt-4" style={{ fontWeight: 500, fontSize: 16, textAlign: 'justify' }}>
                                You are redeeming a voucher to seize the borrower's collateral ({loanDetails?.filCollateral?.amount} FIL).
                            </div>

                            <div className="mt-4" style={{ textAlign: 'justify' }}>
                                <div>• The Borrower has failed to repay the loan before the expiration date and you are able to seize the collateral from the FIL Payment Channel.</div>
                            ...
                            <div className="mt-4">
                                <div>Secret revealed by Borrower (secretA1)</div>
                                <div className="mt-2" style={{ fontWeight: 600, fontSize: 14, overflowWrap: 'break-word' }}>{web3.utils.toUtf8(loanDetails?.erc20Loan?.secretA1)}</div>
                            </div>
...

                    {
                        ...
                        <Fragment>
                            ...
                            <div style={{ padding: '0px 24px 36px 24px', display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column' }}>
                                ...
                                <div style={{ fontWeight: 500, fontSize: 16, marginTop: 5, textAlign: 'center' }}>Redeeming a voucher to seize {loanDetails?.filCollateral?.amount} FIL from Payment Channel</div>
...

                    {
                        ...
                        <Fragment>
                            ...
                            <div style={{ padding: '0px 24px 24px 24px', display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column' }}>
                                ...
                                <div style={{ fontWeight: 400, fontSize: 16, marginTop: 25, marginBottom: 15, textAlign: 'center' }}>You have redeemed a voucher to seize the borrower's collateral. To continue with the unlocking process, you'll need to `settle` the Payment Channel.</div>
...

                    {
                        ...
                        <Fragment>
                            <div className="mt-4" style={{ fontWeight: 500, fontSize: 16, textAlign: 'justify' }}>
                                You are settling the payment channel to seize the collateral ({loanDetails?.filCollateral?.amount} FIL).
                            </div>

                            <div className="mt-4" style={{ textAlign: 'justify' }}>
                                <div>• You have redeemed a voucher and you are now settling the payment channel to seize the funds in it.</div>
...

                    {
                        ...
                        <Fragment>
                            ...
                            <div style={{ padding: '0px 24px 24px 24px', display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column' }}>
                                ...
                                <button style={{ width: '100%' }} onClick={() => this.setState({ modalState: 6 })} className="btn btn_blue btn_lg mt-4">
                                    Next
                                </button>
                            </div>
                        </Fragment>
...

                    {
                        ...
                        <Fragment>
                            ...
                            <div style={{ padding: '0px 24px 24px 24px', display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column' }}>
                                ...
                                <a target='_blank' className="mt-2" href={explorer} style={{ color: '#0062ff', fontWeight: 500, marginTop: 5 }}>View on Filecoin Explorer</a>
                                <div style={{ fontWeight: 400, fontSize: 16, marginTop: 25, marginBottom: 15, textAlign: 'center' }}>You have seized and collected the borrower's collateral from the FIL Payment Channel and the loan is now closed.</div>
